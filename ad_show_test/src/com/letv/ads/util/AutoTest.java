...
public class AutoTest extends BaseCase {
	...
	public void test() {
		...
		System.out.println("-----------测试用例[" + caseName + "]测试开始------------");
		...
		// 准备数据,test-data中文件夹的名称=test-case文件名不带.txt
		String data_fileName = _file.getName().substring(0, _file.getName().lastIndexOf("."));
		scpData(data_fileName);
...
/*		if (test.containsKey("memcached")) {
		...
		System.out.println("-----------测试用例[" + caseName + "]测试结束------------");
	}
}
