...
namespace AdPlugin
{
    public class AdCheck_EUI : Common
    {
        // log标签
            ...
            {// 将report中的参数转存到map
                ...
                for (int i = adResponse.Count - 1; i >= 0; i--)
                {
                 ...
                 //           FiddlerApplication.Log.LogString("-------------------------找到对应广告！！！！");
                            ...
                            s_paramDic.Add("ord", getOrd(ad));
                //            FiddlerApplication.Log.LogString("------------------------------r6");
                            // 校验ac/event上报中的参数是否正确
                            Boolean res = checkParams(r_paramDic, s_paramDic);
               //             FiddlerApplication.Log.LogString("------------------------------r7");
                            if (res)
                            {
                                //event上报
                                if ("event".Equals(act))
                                {
                                    msg = "广告event上报发送成功，cuepoint_type=" + ad["cuepoint_type"].ToString() + ",order_item_id=" + oiid + ",id=" + r_paramDic["id"];
                                }
                                else
                                {//ac上报
