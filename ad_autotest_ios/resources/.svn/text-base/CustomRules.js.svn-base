...
class Handlers
{
    ...
    */
    ...
    static function OnBeforeRequest(oSession: Session) {
        ...
        //去掉protobuf参数后，虽然能获取json返回串，但客户端无法正常播放广告，因此不使用该方法
        /*      if (oSession.uriContains("/play?")) {
          
        // 将修改url中的protobuf参数
        oSession.url = oSession.url.Replace("protobuf=1","protobuf=0");
        oSession.url += "&_debug=1";
            
        //2.写入autoTest.txt
        var fso = new ActiveXObject("scripting.FileSystemObject");
        var path = "D:\\temp\\";
        var report = path +"autoTest_Report.txt";
        ...
        var tf = fso.OpenTextFile(path,8,true);
        tf.WriteLine(oSession.url);
        tf.Close();
            
            
        //写入autoTest_Report.txt         
        if(!fso.FileExists(report)){
        fso.CreateTextFile(report,true);
        } 
        var tf2 = fso.OpenTextFile(report,8,true);
        tf2.WriteLine(oSession.url);
        tf2.Close();
        }   
        */   
    ...
    */
	...
	/**    
	*   将广告信息、监测和上报数据写到d:\temp\autoTest.txt 
    *    
    *   play repsonse 如果是protobuf格式，则写入文件d:\temp\playResponse.txt
    *
    */
        
    static var lock: boolean = false;   //锁标识
    ...
    static function OnBeforeResponse(oSession: Session) {
        ...
        //加载解析配置文件的过程中，不输出日志信息（解决fiddler提示的出错）
        //  CONFIG.QuietMode=true;   
        	
        // 页面广告
        if ((oSession.HostnameIs("test.ark.letv.com") || oSession.HostnameIs("ark.letv.com"))
        &&  oSession.uriContains("/s?")) {
...
　		
            ...
            var show_response = oSession.GetResponseBodyAsString();
            ...
            while(true) {
                ...
                if(!lock) {
                    ...
                    var tf = fso.OpenTextFile(path,8,true);	
                    tf.WriteLine(oSession.url);
                    tf.WriteLine("show_response-----"+show_response.Trim());
                    tf.Close();
        ...
        //播放器广告，（前贴 标板 中贴  ct=2,3,4）
        if ((oSession.HostnameIs("t.api.mob.app.letv.com") || oSession.HostnameIs("api.mob.app.letv.com"))
        && oSession.uriContains("/play?")) {

            var play_response  = "";
            
            if(oSession.uriContains("protobuf=1")){
                oSession.SaveResponseBody("D:\\temp\\playResponse.txt");
            
            }else{
                play_response = oSession.GetResponseBodyAsString();
                var i1 = play_response.IndexOf("{");
                var i2 = play_response.LastIndexOf("}");         
                play_response = play_response.Substring(i1,i2-i1+1);
            }
            ...
            while(true) {
                if(!lock) {
                    ...
                    var tf = fso.OpenTextFile(path,8,true);
                    tf.WriteLine(oSession.url);
                    tf.WriteLine("play_response-----"+play_response);
                    tf.Close();     
        ...
        // 数据监测(tracking)，忽略焦点图的曝光，mid=13279 & 13281
        if (oSession.HostnameIs("ark.letv.com") && oSession.uriContains("/t?mid=")
        && !(oSession.uriContains("/t?mid=13279") || oSession.uriContains("/t?mid=13281"))) {
...
　			
        ...
        // 数据上报
        if ((oSession.HostnameIs("dev.dc.letv.com") || oSession.HostnameIs("apple.www.letv.com"))
            && oSession.uriContains("/va/?act=") && !oSession.uriContains("act=traf")
            && !oSession.uriContains("act=block") && !oSession.uriContains("act=eblock")) {
